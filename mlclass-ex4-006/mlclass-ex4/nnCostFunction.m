...
for i=1:m
    j=y(i);
    y2(i,j)=1;
    J=J+1.0/m*(-y2(i,:)*log(a3(i,:))'-(1-y2(i,:))*log(1-a3(i,:))');
end

J=J+0.5*lambda/m*(sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2)));

%J=1.0/m*sum(-y2'*log(a3)-(1-y2)'*log(1-a3));
...
for i=1:m
    ...
    %delta1=(delta2*Theta1(:,2:end)).*sigmoidGradient(z1);
    ...
    Theta1_grad(:,1)=1.0/m*DELTA1(:,1);
    Theta2_grad(:,1)=1.0/m*DELTA2(:,1);
